...
int compute(float *img, float *f, float * out, int bh, int bw, int imgH, int imgW, int imgN, int nF, int convH, int convW) {
    ...
    int con, imgg, inm1, inm2, inm3, inm4, inf, ind1, ind2, ind3;
    inm1 = 0;
    inf = 0;
    ind1 = 0;

    for (int mi = 0; i < imgN; i++){
        ind1 += convW * convH;
        inm1 += imgW * imgH;
...
__global__ void compute_gpu_csr(float *img, float *out, int bh, int bw, int imgH, int imgW, int imgN, int nF, int convH, int convW){
    ...
    for(int k=0; k<convDims; k++){
        ...
        for(int i=0; i<nF; i++){
            for (int j=0; j<nF; j++){
                if (h_img[i*nF+j+k] != 0){
                    coor[index_p+k] = j;
                    h_fcsr[index_p+k] = filter[i*nF+j+k];
                    index_p++;
                }
            }
            pos[i] = index_p;
